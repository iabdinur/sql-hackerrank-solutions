...
to buy each non-evil wand of high power and age. Write a query to print the id, age, coins_needed, and power of 
...
galleons needed to buy the wand, and power denotes the quality of the wand (the higher the power, 
...
on w.code = wp.code
where is_evil = 0
and w.coins_needed = (SELECT min(w1.coins_needed) from wands w1 join wands_property wp1 on w1.code = wp1.code where wp.age = wp1.age and w.power = w1.power)
